...
use crate::{
    ...
    errors::user_error,
    github::{Event, IssueCommentEvent, client::GraphQlErrors},
    handlers::Context,
};
...
async fn delegate_to(
    ...
    state.data.delegations.push(delegate_to.to_string());
    ...
    issue_comment
        .issue
        .post_comment(&ctx.github, &format!(r#":v: @{delegate_to}, you can now merge this pull request!
...
async fn was_delegated_to_commenter(
    ...
    for delegated_login in &state.data.delegations {
        if delegated_login == &issue_comment.comment.user.login {
            return Ok(true);
        }
    }

    // Manually drop the state to release the DB lock
    drop(state);
